...
describe("Selector parsing", () => {
	...
	test("parseSelectors handles non-string input", () => {
		...
		expect(parseSelectors(123 as unknown as string)).toEqual([]);
		expect(parseSelectors({} as unknown as string)).toEqual([]);
		expect(parseSelectors([] as unknown as string)).toEqual([]);
	});
	...
	test("parseSelectors keeps commas inside :is() and attribute selectors", () => {
		const input = ":is(.one,.two), [data-value='a,b'], .three";
		const result = parseSelectors(input);
		expect(result).toEqual([":is(.one,.two)", "[data-value='a,b']", ".three"]);
	});
});
